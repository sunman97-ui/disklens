...
class TreemapView(ttk.Frame):
    ...
    def _draw(self):
        ...
        if w < 2 or h < 2 or not self._cur:
            return
        sizes = [i["size"] for i in self._cur]
        if not sum(sizes):
            return
        rects = squarify.squarify(
            squarify.normalize_sizes(sizes, w - 2, h - 2), x=1, y=1, dx=w - 2, dy=h - 2
        )
        for i, (item, r) in enumerate(zip(self._cur, rects, strict=False)):
            ...
            if pw > 40 and ph > 20:
                c.create_text(
                    ...
                    text=item["label"],
                    fill="white",
                    font=("Segoe UI", max(8, min(13, int(pw / 10)))),
                    width=pw - 8,
                    tags=tag,
                )
                c.create_text(
                    ...
                    text=_fmt(item["size"]),
                    fill="#cccccc",
                    font=("Segoe UI", max(7, min(10, int(pw / 14)))),
                    tags=tag,
                )
